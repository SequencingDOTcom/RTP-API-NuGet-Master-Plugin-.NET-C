...
namespace Sequencing.MusterPluginSample.Web.Controllers.OAuth
{
    ...
    public class AuthWorker
    {
        ...
        public AuthWorker(string oAuthUrl, string redirectUrl, string secret, string appId, string scope)
        {
        ...
        public DrupalOAuthInfo GetUserInfo(string token)
        {
            ...
            {
                throw new Exception(string.Format("Invalid access token {0}. {1}", token, e.Message));
            }
        }
        ...
        public AuthInfo GetAuthInfo(string code)
        {
            ...
            try
            {
                ...
                using (var _sw = new StreamWriter(_str))
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("grant_type={0}&code={1}&redirect_uri={2}", "authorization_code", code,
                        HttpUtility.UrlEncode(redirectUrl));
                    _sw.Flush();
                }
                var _webResponse = _webRequest.GetResponse();
        ...
        {
            return oAuthUrl + "?q=oauth2/authorize&redirect_uri=" + redirectUrl + "&response_type=code&state=123&client_id=" + appId + "&scope="+scope;
        }
    }
}